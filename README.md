...
| **go-scaffold**         | Scaffolds a Go REST API — Gin, contract-first oapi-codegen, GORM + Postgres, JWT access/refresh auth + RBAC. Runs codegen + build/vet/test itself.         |
...
| `go`     | Go REST API — Gin, contract-first (`oapi-codegen`: `openapi.yaml` → `api/api.gen.go`), GORM + Postgres, JWT access tokens + rotating refresh tokens + RBAC, `golang-migrate`, `air`. Empty repo → scaffolded by the `go-scaffold` skill                     |
...
**go on an empty repo:** scaffolded **by the `go-scaffold` skill's deterministic script** — module path + project name gathered upfront as CLI flags, then `node scripts/scaffold.mjs --module <path>` writes a flat-package Gin API: `openapi.yaml` as the contract, `handlers/` implementing the generated `api.ServerInterface`, `middleware/` (JWT + RBAC, per-route rate limiting, origin-allowlisted CORS, path selectors), `models/` (GORM), `config/`, `utils/`, and `migrations/` (`golang-migrate`). The auth kernel covers signup with password-complexity + HTML-rejection validation, login, refresh-token **rotation** (opaque, stored hashed, replay-detecting), logout, profile, and admin user management with self-demotion/self-deletion blocked. Runs `oapi-codegen` from `openapi.yaml` via `go run pkg@version` (no global install, no `go.mod` pollution), then `go mod tidy` + `gofmt` + `go vet` + `go build` + `go test` + optional `staticcheck` + `git commit` — all before reporting success. Routing is generated but **security is not**: `middleware/selector.go` maps path prefixes to roles, and `main_test.go` asserts both directions of that wiring against the real router. `go-scaffold` writes no `.claude/` anything; the harness governance layer is overlaid additively afterward, same as nuxt.
...
│   │       └── artifacts.md       ← rationale + merge semantics for the templates
│   ├── go-scaffold/               ← Go REST API scaffolder (Gin, oapi-codegen, GORM + Postgres, JWT access/refresh + RBAC)
