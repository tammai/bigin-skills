...
| `go`     | Go modular-monolith REST API — Gin, contract-first (`oapi-codegen`: `openapi.yaml` → `internal/openapi/openapi.gen.go`), GORM + Postgres, JWT access tokens + rotating refresh tokens + RBAC, `golang-migrate`, `air`. Empty repo → scaffolded by the `go-scaffold` skill                     |
...
**go on an empty repo:** scaffolded **by the `go-scaffold` skill's deterministic script** — module path + project name gathered upfront as CLI flags, then `node scripts/scaffold.mjs --module <path>` writes a **modular-monolith** Gin API: `openapi.yaml` as the contract, `cmd/server/` as the composition root, and `internal/` holding the generated `openapi/` package, an `api/` layer (router, health probes, module assembly, and `middleware/` for JWT + RBAC, per-route rate limiting, origin-allowlisted CORS, path selectors), one `modules/users/` module split into `domain`/`application`/`infrastructure`/`api` behind a `module.go` public contract, and a `shared/` kernel (`apperr`, `auth`, `config`, `db`, `httpx`, `validate`). The auth kernel covers signup with password-complexity + HTML-rejection validation, login, refresh-token **rotation** (opaque, stored hashed, replay-detecting), logout, profile, and admin user management with self-demotion/self-deletion blocked. Runs `oapi-codegen` from `openapi.yaml` via `go run pkg@version` (no global install, no `go.mod` pollution), then `go mod tidy` + `gofmt` + `go vet` + `go build` + `go test` + optional `staticcheck` + `git commit` — all before reporting success. Two things are enforced rather than documented: **module boundaries**, by `internal/arch` failing `go test ./...` on an import that reaches past a module's `module.go` or points the wrong way through the layers; and **security**, which is *not* generated even though routing is — `internal/api/middleware/selector.go` maps path prefixes to roles, and `internal/api/router_test.go` asserts both directions of that wiring against the real router. `go-scaffold` writes no `.claude/` anything; the harness governance layer is overlaid additively afterward, same as nuxt.
...
**Already on GitHub Spec Kit?** The skill detects it (both layouts — the current one ships skills under `.claude/skills/speckit[-.]*`, the older one ships slash commands and maintains a block in `CLAUDE.md`) and offers three paths: `migrate` (triage `specs/`, salvage contracts and rationale, remove Spec Kit, install the harness as the only workflow), `coexist` (governance only — the harness's guards, budget gate and CI, with the spec-gate hook left unregistered so it can't block Spec Kit's implementation step), or `leave`. Migration never deletes anything before you've seen a read-only triage table of every feature under `specs/`. Details in [`references/speckit-migration.md`](skills/bigin-harness-setup/references/speckit-migration.md).
...
3. **Plan file** — the approved spec plus a tasks-tracking table, written to `PLAN.md`. This is exactly what `spec-gate-guard.mjs` checks for: it blocks `Edit`/`Write`/`MultiEdit` calls until `PLAN.md` exists with `Status: approved` and a `Branch:` line matching the branch you're on. Before implementing, the plan is read back once for coverage — every requirement has a task, every task has a requirement, every named edge case is covered by a task or the testing strategy. A gap costs a line to fix here and a whole verify round after code exists. The table is then mirrored into Claude Code's task list for live progress, one-way and disposable — `PLAN.md` stays the source of truth, since the verifier and the guard read the file off disk and can't see session state at all.
...
6. **Cleanup** — once every task is `Done` and review is resolved (clean, or explicitly declined), `PLAN.md` is deleted. It's a working file for the task, not project documentation. Two things are proposed first: distilling anything durable the task established — a decision, invariant, contract, or constraint — into the `knowledge/` bundle, since deleting the plan is the last chance to keep it; and a graph rebuild if the repo has one.
...
│   │       └── artifacts.md       ← rationale + merge semantics for the templates
│   ├── go-scaffold/               ← Go modular-monolith REST API scaffolder (Gin, oapi-codegen, GORM + Postgres, JWT access/refresh + RBAC)
...
│   │       └── templates/files/   ← source of truth; whole tree written in one pass, then codegen
