...
- **`agent-teams` skill — support for Claude Code agent teams, where several teammates share one working tree.** Teams have **zero file isolation** (task *claiming* is file-locked; file *writes* aren't), so the skill's job is the protocol that makes them safe: the isolation-vs-coordination decision up front (isolated `isolation: "worktree"` subagents are recommended whenever teammates don't need to talk to each other — that path has none of the shared-tree hazards), sharding by file ownership, plan-before-task-before-teammate ordering, `plan_mode_required` as the native spec gate, per-task scoped verification, and shutdown/cleanup. References cover the verified platform contract (`platform-facts.md`), the ownership mechanism (`ownership-protocol.md`), and our four agents as teammates (`roster.md`).
- **`.claude/task-plans/<slug>.md` — per-task plans with explicit file ownership.** Two new header lines: `Owns:` (comma-separated globs a task exclusively owns) and `Verified: PASS <iso8601>` (written after a verifier passes). Ownership is scoped by **path, not actor**: measurement showed all teammates share the lead's `session_id`, and while a teammate's `PreToolUse` payload does carry its name in `agent_type`, that name is silently de-duplicated (`alpha` → `alpha-2`) and means something different for a plain subagent — so the name is used for message attribution only, never authorization.
- **`spec-gate-guard.mjs` scoped mode.** With any plan declaring `Owns:`, the gate resolves ownership by path: exactly one approved plan must own the target, an **unowned path is blocked at any size** (the ≤20-line hatch deliberately does not apply — a 5-line edit to an unclaimed file is precisely the silent overwrite this stops), two plans at equal specificity are blocked (this doubles as the ownership-collision detector), and an unparseable `Owns:` fails **closed**. Precedence is specificity, so an `Owns: **` catch-all always loses to a specific glob. **Legacy mode — no plan declaring `Owns:` — is byte-identical to before**, which is the compatibility contract for every already-scaffolded repo.
- **Two opt-in team gates**, `task-plan-gate.mjs` (`TaskCreated`: the task must reference a plan that is `Status: approved` with a non-empty `Owns:`) and `task-verify-gate.mjs` (`TaskCompleted`: that plan must carry `Verified: PASS`). Both **exit 0 when `.claude/task-plans/` is absent** — the task tools fire constantly in ordinary non-team sessions, and without that check scaffolding these guards would break `TaskCreate` everywhere. `TaskCompleted` deliberately fails **open** on a missing plan file: cleanup-then-complete is legitimate, and a task that can never be completed deadlocks its dependents. There is deliberately **no `TeammateIdle` gate** — exit 2 there re-prompts the teammate, and the only available checks describe exactly the state a teammate legitimately waits in.
- `bigin-harness-setup`: new `AGENT_TEAMS` decision in Phase 1.5 (default **off** — experimental, token-hungry, `/resume` doesn't restore teammates). When on, Phases 5-2h and 5-3e write the two gates, merge `env.CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` plus the two hook events, create `.claude/task-plans/`, and write a ≤40-line unscoped `.claude/rules/agent-teams.md`.
- `classify.mjs`: `--paths <glob,glob>` and `--plan <path>` scope the rubric signals to one agent's owned paths, with a new `scopedTo` field reporting `{source, globs, plan, excluded}`. Unscoped runs behave exactly as before. Without this, a shared tree attributes every teammate's dirty file to "this task" — inflating `filesChanged` and firing the `highRiskMatches` deep-tier override on work the agent never touched. `--plan` also checks *that* plan for the full-spec signal, so one teammate's full-spec plan no longer auto-routes every sibling to Deep.
- `tools/team-probe.mjs` — measures the agent-teams hook contract rather than trusting the docs (`--settings` prints the block to merge, `--report` reduces the log to answers). It records values only for an allowlist of non-secret `CLAUDE_*` vars; every other one is logged by name only.

### Changed

- **Corrected every claim agent teams falsify.** A definition's `effort:` is **not** applied to a teammate (they inherit the lead's effort) while `model:` still is; `skills:`/`mcpServers:` are dropped on that path, so `standard-worker` must now invoke `debug-workflow`/`write-tests` explicitly instead of relying on frontmatter; `verifier` gains messaging/task tools in team mode regardless of its `tools:` allowlist. Updated across `model-router`'s SKILL + both references, all four `agents/*.md`, and `.claude/rules/skill-authoring.md`.
- **`parallelization.md` rewritten.** It previously resolved concurrency by prohibition — *"Never point two instances at the same working tree"* — which reads as a ban on agent teams. It now names three models (worktree-per-instance, isolated subagents, agent teams), says which to pick, and corrects the harness interaction per model: per-worktree `SESSION.md` and per-worktree spec-gate approval hold for the first two and **not** for teams, where a single global approval would leak across every agent.
- `task-workflow`: the verifier now receives a diff scoped to the plan's `Owns:` globs (`git diff -- ':(glob)…'`) instead of "the diff", which in a shared tree is everyone's work; `PLAN.md` deletion is forbidden while another agent still owns work against it; the `PLAN.md` format section documents the team-only `Owns:`/`Verified:` lines.
- `session-handoff` records team composition, ownership and task state, and states that `/resume` does **not** restore in-process teammates — the lead must respawn them from that table.

### Fixed

- `classify.mjs` caps the emitted `touchedFiles` list at 50 entries (adding `touchedFilesTruncated` with the real total) while `filesChanged` keeps the true count. The list is pasted into a subagent's payload, and untracked-but-unignored files shouldn't be able to flood it — the rubric's top band is "5+", so a count of 60 and 600 route identically anyway.
- **`classify.mjs` under-counted new files and hid them from `--paths`.** `git status --porcelain` collapses an untracked directory into a single entry, so a new 12-file module counted as 1 file, and a glob naming anything *below* that directory matched nothing — meaning a teammate's own new files fell outside its own scope. Now uses `-uall`. This can raise `filesChanged` for repos with new directories and route a task one tier up; that is the correct count.
- The `verifier` never saw brand-new files. A new file doesn't appear in `git diff`, so the audit ran against a diff with the new module missing — a pre-existing bug, not a team one. The handoff now adds `git diff --no-index -- /dev/null <path>` per untracked owned path, deliberately **not** `git add -N`, which would mutate the index every teammate shares.
- A literal `\x01`/`\x00` control character had been used as a sentinel inside the glob-to-regex converter — invisible in review, and it would corrupt any glob containing that byte. Replaced with a single-pass scanner in both `classify.mjs` and the guard template.
- **Caught by the single-agent regression pass, before release:** Phase 5-3e originally created `.claude/task-plans/` with a `.gitkeep`. Since that directory's existence is what arms both task gates, every ordinary `TaskCreate` in a team-enabled repo — including in plain solo sessions that never form a team — would have been blocked demanding a `Plan:` reference. The directory is now created by the lead with the first plan, so the gates stay genuinely inert until a team runs.
- **Also caught pre-release:** `Owns:`/`Verified:` were matched anywhere in a plan file, so an existing `PLAN.md` whose spec body happened to start a line with `Owns:` would have flipped that repo into scoped mode and blocked every unowned path. Both are now read from the header region only (before the first `##`). `Status:` still scans the whole file — that is the legacy behavior and changing it would alter how existing repos are gated.

### Added (tooling)

- `tools/team-probe.mjs` now captures the payload's `effort`/`model`/`permission_mode` and reports **effort per agent from the payload**, which is the only way to settle "do teammates inherit the lead's effort" on the in-process backend where every agent shares one `CLAUDE_EFFORT`. Two bugs fixed in the same pass: the payload `effort` is an object (`{level:"high"}`), so the previous comparison compared object identities and declared every record a different effort; and `--report` now prints a loud warning when a log contains no team events and no teammate-format agent ids, because **headless `claude -p` does not form a team** — it silently falls back to plain subagents, and every team conclusion drawn from such a log is measuring the wrong thing.

- `tools/guard-tests/` — the guard test cases in `.claude/rules/skill-authoring.md` are now executable. `node tools/guard-tests/run.mjs` extracts every guard **from `hook-guard.md`** (so the suites test what actually gets scaffolded, not a copy that drifted) and runs two suites: `regression.mjs` (24 cases — single-agent behavior that must never change, including `bash-guard`'s allow/block set and the team gates staying inert) and `matrix.mjs` (38 cases — the full legacy + scoped + team-gate matrix).

### Notes

Existing scaffolded repos are unaffected until they opt in: `spec-gate-guard.mjs` stays in legacy mode until some plan declares `Owns:`. To enable teams in an already-scaffolded repo, re-run `bigin-harness-setup` and answer **yes** to the new agent-teams question — this ships no `patch` blocks on purpose, since half-applying the gates (task gates active, spec-gate still legacy) would enforce plans without enforcing ownership.

Verified on Claude Code 2.1.219 against two real two-teammate runs; the measurements, and the questions still open (`tmux` backend, teammate `AskUserQuestion`, effort inheritance), are recorded in `skills/agent-teams/references/platform-facts.md`.
