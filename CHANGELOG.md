...
- **`contract-drift` no longer runs on every PR.** It re-runs codegen to prove the committed generated client still matches the vendored spec — the one check in this design with no local equivalent, and on `mobile` one that pulls a ~1 GB generator image. Paying that on a PR which touched only UI code buys nothing.
...
- **`nuxt-marketing-scaffold` produced a repo that did not build.** Four defects, each fixed against a real `pnpm install && pnpm lint && pnpm type-check && pnpm build` rather than against a reading of the templates.

  **The locale set was silently discarded.** `scaffold.mjs`'s substitution regex was `/__([A-Z_]+)__/g`, and `[A-Z_]` cannot match the digits in `I18N`, so `__LOCALES_I18N__` was written through into every generated `nuxt.config.ts` verbatim and the config died with `__LOCALES_I18N__ is not defined` before Nuxt started. Passing a caller's `--locales` through is this scaffolder's main job, and it had never once done it. The class is now `[A-Z0-9_]`. The same species had eaten `--primary` and `--neutral` by a different route — see the theme fix below.

  **pnpm 10 refused to build `better-sqlite3`, and the pinned range was incompatible anyway.** `packageManager` pins pnpm 10, which runs no dependency install script unless the package names it in `pnpm.onlyBuiltDependencies`; the manifest declared none, so the native binding never compiled and `@nuxt/content` died at `Could not locate the bindings file`. The five packages that actually want a build script — `better-sqlite3`, `esbuild`, `unrs-resolver`, `vue-demi`, `workerd` — are now declared, verified against a clean install rather than transcribed. Separately, `@nuxt/content@3.16` asks for `better-sqlite3` `^12.5.0 || ^13.0.0` against a pinned `^11.8.0`; the pin is now `^13.0.0`.

  **`tsconfig.json` was the Nuxt 3 shape.** `{ "extends": "./.nuxt/tsconfig.json" }` is now the four project references Nuxt 4 generates. This did not break the build — the reported symptom did not reproduce, and Nuxt 4.5 does still emit `.nuxt/tsconfig.json` — but it was wrong in a quieter and worse way: with one merged project, server code is type-checked against the *app*'s `lib`, so `document.title` inside a Worker route type-checks clean and crashes at runtime. Under the references shape it is `TS2584: Cannot find name 'document'`. `nuxt typecheck` reads the root config to decide between `vue-tsc --noEmit` and `vue-tsc -b`, so both shapes "work"; only one of them checks the right globals.

  **Five symbols the templates called did not exist.** `verifyTurnstile`, `rateLimit`, `deliverContactMessage`, `subscribe` and `blockFor` were called by the two form routes and the catch-all page, and defined nowhere; `zod` was imported by both routes and declared in no manifest. They are implemented rather than deleted or stubbed, because the template's own comment is right that Turnstile, rate limiting and validation are requirements on those routes. All four server helpers live in `server/utils/` (Nitro auto-imports it) and every one of them **fails loudly when unconfigured** rather than degrading: an absent Turnstile secret is a 503 `TURNSTILE_SECRET_MISSING`, an absent `RATE_LIMIT` KV binding is a 503 `RATE_LIMIT_BINDING_MISSING` — never a silent allow — and an unset delivery endpoint is a 501 naming the variable to set. A form that returns `{ ok: true }` into a void is the worst failure these routes can have, because nobody finds out until a client asks why nobody replied. `blockFor` is a component registry in `app/utils/`: an unknown block type renders nothing, the same rule the profile already applies to a missing locale.

  Three more defects surfaced while verifying the four. **`ui: { theme: { colors: { primary, neutral } } }` in `nuxt.config.ts` is the wrong shape** — `@nuxt/ui`'s `theme.colors` is a list of alias *names*, so an object has no `length`, falls back to the defaults, and the `--primary`/`--neutral` flags were discarded exactly as `--locales` was. Values belong in `app.config.ts`, which the scaffolder now writes. **`@nuxt/ui` ships no stylesheet**, so without an `app/assets/css/main.css` importing `tailwindcss` and `@nuxt/ui`, and a `css:` entry pointing at it, Tailwind never ran and every utility class in the block tree was inert. And **`nitro.prerender.routes` was `['/']`** with nothing on the page to crawl, so a multi-locale site prerendered exactly one locale — the failure the profile's own CI step exists to catch. It is now one explicit entry point per locale.

  Also: `postinstall: nuxt prepare` (without it a fresh clone has no `.nuxt/eslint.config.mjs` and CI's `pnpm lint` fails before it starts), `eslint.config.mjs` written rather than left for the module to generate, the `typecheck` script renamed to `type-check` to match the name the profile's `CLAUDE.md` and CI both call, and `generate` dropped since the profile is explicit that the build is the command.

- **Phase 0.5 told the harness to skip the scaffolder that had just been added to it.** `bigin-harness-setup`'s `SKILL.md` still carried the pre-1.88.0 sentence "**Also skip it entirely for `PROFILE = nuxt-marketing`** — there is no marketing-site scaffolder here, which is why row 8's question does not offer the profile", while `references/scaffold-delegation.md` carried the delegation row, the empty-repo question carried option 7, and `regress.mjs` asserted both. An empty `nuxt-marketing` repo taken through the documented phase order would therefore have been onboarded with no app in it. `references/idempotency.md` and `references/summary-checklist.md` said the same thing, and `docs/USER_GUIDE.md` said it in two places, one of them in a sentence that also named `nuxt-marketing-scaffold` as the thing that runs. `idempotency.md` additionally described the detection rung as three conditions whose third is "no `server/api/**`" — the rung is four conditions and `server/api/**` is deliberately not among them, which is the one thing about it most worth not getting wrong. All six surfaces corrected, and a WIRING case added so the claim cannot come back.

- **`pnpm install`, not `pnpm add`, and one manifest instead of two dependency lists.** `scaffold.mjs` carried `DEPS`/`DEV_DEPS` arrays and installed with `pnpm add <names>`, which re-resolves every name to whatever is latest that morning and overwrites the manifest's pinned ranges. So `--no-install` and a real install produced two different dependency sets, and only one of them was ever built. The arrays are gone; `package.json.tmpl` is the single source and the install step reads it.

### Added

- **`tools/regress.mjs` now checks that the scaffolder's output could build — and, on request, that it does.** The v1.88.1 suite scaffolded a site and asserted it *detected* as `nuxt-marketing`. That is how four build-breaking defects passed a green suite: nothing anywhere asked whether the repo compiled.

  Four structural cases, always on, in the commit hook, costing milliseconds. Every requested locale reaches both `i18n.locales` and the prerender routes. `--primary`/`--neutral` reach `app.config.ts`. Every bare import specifier in the generated tree resolves to a declared dependency — with `vue` and `h3` allowed only as `import type`, since Nuxt's generated `paths` aliases carry a type-only import and pnpm's strict layout will not resolve a value one. And every lowercase call in the generated tree is either a declared local, a listed Nuxt auto-import, or a helper exported from `app/utils/`, `app/composables/` or `server/utils/`. That last one is maintenance by design: a newly used Nuxt built-in fails the suite until it is added to the list, which costs one line and is the price of catching a dangling symbol.

  **The pre-existing placeholder check was itself an instance of the bug.** `no leftover __TOKEN__ placeholders` was written with `/__[A-Z_]+__/` — the same character class as the defect — so it could not see `__LOCALES_I18N__` either, and it scanned only the single-locale scaffold. An assertion copied from the code it audits asserts nothing. It now uses a deliberately wider class and scans every scaffold the suite produces.

  **`node tools/regress.mjs --build` is the case that actually compiles.** It scaffolds three locales, really installs, runs `lint`, `type-check` and `build`, and asserts one prerendered `index.html` per locale, each rendered from its own content file. It is the only check that can catch a peer-range conflict, a missing build-script approval or a wrong config shape, because those need a resolver and a compiler. It is opt-in — the network and roughly three minutes are not a commit hook — and when it does not run it prints `SKIP` with the reason and is counted in the summary, since a slow check nobody notices is missing is how this shipped in the first place. It skips visibly, never silently, when `--build` is absent, when `pnpm` is not on `PATH`, or when the registry is unreachable.

  What this does **not** prove: nothing here deploys, so `wrangler.jsonc`'s bindings, the Turnstile round trip and the delivery seams are exercised by neither half. The structural cases are regex over text and will miss a defect that is well-formed but wrong; the build case runs on one platform with one lockfile resolution at one moment in time. Each of the four defects was verified caught by mutating the fix away and watching the suite go red.
...
- **An eighth stack profile, `nuxt-marketing`: multi-locale Nuxt marketing sites.** `@nuxt/content` collections, `@nuxtjs/i18n`, Tailwind, prerendered onto Cloudflare Workers static assets. No auth, no BFF, no database.
  ...
  **Detection is a new rung above `nuxt`, for the same first-match-wins reason `tauri` sits there.** Four conditions, all required: `nuxt.config.ts`/`.js` present, `@nuxt/content` in `dependencies`, `@nuxtjs/i18n` in `dependencies` with a `content/` tree or `content.config.ts`, and no auth marker (neither `nuxt-auth-utils` nor `@sidebase/nuxt-auth`). Conditions 3 and 4 are the narrowing test and they are what leaves the `nuxt` rung untouched — a Nuxt fullstack app that ships a docs section satisfies the first two and must stay on `nuxt`; `@nuxt/content` in `devDependencies` only is not a match either. `server/api/**` is deliberately **not** tested: the profile's own contact-form and newsletter routes live there, one Worker per site, so testing for its absence would be correct at install and wrong from the first contact form onward. A Tauri app whose Nuxt frontend uses `@nuxt/content` still resolves to `tauri`, which is higher again. The empty-repo question is deliberately **not** extended: there is no marketing-site scaffolder here, and offering a choice this skill cannot scaffold is the failure mode `profile-generic.md` already names for Flutter packages. The profile is detection-only.
...
- **`ask-bigin` was missing from every hand-maintained surface.** v1.81.0 added the skill and regenerated the README tables, which is mechanical — `docs_sync.mjs` reads the manifest. What it did not do is the docs sweep a minor bump is supposed to trigger, so the only mention of `ask-bigin` anywhere was that generated table row. For a skill whose entire purpose is "you don't have to know which skill", being absent from the docs is a functional defect, not just missing coverage.
  ...
  It also gets the entry that matters most, in `USER_GUIDE.md` §5's overlap section (now five pairs, not four): **`ask-bigin` vs just saying what you want.** If you can state the work, state it — the skill you need triggers on it directly, and routing first adds a hop to answer a question you had already answered. `ask-bigin` is for not knowing what's available, being between two skills, or wanting the inventory. It routes and stops, so it is never the faster path to work you can already name.
...
- **Harness audit against current docs: six drift/opportunity findings fixed.** First audit since v1.35.1 (2026-07-15), so the sweep covered 44 minor versions of accumulated drift.

  **The go profile's permission allowlist was pre-approving a wildcard task runner.** `Bash(make:*)` let any Makefile target run unprompted — including whatever the repo adds to it later — and `Bash(migrate:*)` pre-approved golang-migrate directly. The seven documented targets are now enumerated (`build`, `run`, `dev`, `lint`, `test`, `generate`, `migrate-create`) and the bare `migrate` entry is gone. **`make migrate-up` is deliberately not on the list**: applying a migration changes schema state, and this harness already treats schema changes as reviewable, so it should cost a prompt. `make migrate-create` only writes a SQL file, so it stays. The rationale sits above the template so nobody collapses it back into a wildcard. This is the only finding with downstream blast radius — setup *merges* missing allowlist entries and never removes one, so an already-scaffolded go repo keeps `Bash(migrate:*)` until the patch block below runs.

  **The `PostToolUse` tool-output field was documented as a host difference when it is a version difference.** `hook-guard.md`'s Claude-Code-vs-Cursor table mapped tool output as `tool_response` → `tool_output`; current `hooks.md` documents `tool_output` as Claude Code's own field name, with `tool_response` the older name (the 2026-07-15 audit verified `tool_response` against v2.1.209, so the rename landed between then and v2.1.247). Behavior was never affected — `lib/hook-io.mjs` reads `tool_response ?? tool_output`, so one guard body already spanned both — but the table taught the wrong mapping and `cursor-parity.md` repeated it. Both corrected, with a note that the fallback exists for the rename and must not be "fixed" back into a host split.

  **`.claude-plugin/plugin.json` was missing `repository`, `license`, and `homepage`** — all three present in `.cursor-plugin/plugin.json` and in the `.claude-plugin/marketplace.json` entry, absent only from the file this repo calls the source of truth.

  **CLAUDE.md's pre-bump sweep pointed at a README tree diagram that doesn't exist.** README has no tree; CLAUDE.md's own Structure block is the manual surface that can actually go stale, so the instruction now names that.

  **`argument-hint:` added to the ten skills you can usefully type an argument after** (`knowledge-distill` → `[library@version]`, `nuxt-ui-figma-handoff` → `[figma file or frame url]`, and so on). Autocomplete-only and outside the always-loaded description, so the budget gate is unmoved at 8274 chars. No skill substitutes `$ARGUMENTS`; arguments arrive appended to the body, which is why the hint is worth having even where nothing interpolates.

  **`maxTurns:` added to the three read-only auditors** — `verifier` and `verifier-medium` at 60, `knowledge-auditor` at 80 — as a **runaway backstop, not a working limit**. The audit finding originally framed this as the deterministic form of the callers' 3-round cap; that was wrong on the axis. `maxTurns` bounds turns inside one invocation, so a value low enough to enforce a round cap would truncate an audit mid-way and return a partial read as though it were a verdict. Each agent file now says so in place.

  **Not done, deliberately: moving trigger phrases from `description:` to `when_to_use:`.** It looks like the tidier home and the audit listed it as an opportunity, but `when_to_use` is a Claude Code extension and not one of the Agent Skills spec's six fields — and the Cursor manifest points at these same skill directories. Moving matching-critical text into a field a non-Claude host isn't guaranteed to read would weaken triggering there with nothing failing visibly, and it saves no budget since both fields share one listing cap. Recorded as a standing rule in `skill-authoring.md` so it isn't re-proposed.

  Also verified rather than assumed: `session-resume-check.mjs`'s `hookSpecificOutput.additionalContext` on `SessionStart`. The current `hooks.md` output schema for that event lists only `systemMessage`/`terminalSequence`, and the binary's own SessionStart examples use plain stdout — so it was tested empirically against v2.1.247 with a canary hook in a scratch repo. The token reached the model; the guard is correct and the docs summary was incomplete.
...
```
  ...
  The patch block above targets the **go** profile's allowlist only. Patching a nuxt/next/nodejs/flutter/generic repo will report it under "Skipped (anchor not found)" — that is expected, not a problem to chase.

### Fixed

- **Pre-minor-bump docs sweep** (required by CLAUDE.md before a minor). Five stale items across the manual surfaces:

  **CLAUDE.md's own Structure tree** listed `tools/context_budget.mjs` and nothing else under `tools/`, omitting `docs_sync.mjs` (generates README's tables, runs in pre-commit) and `docs-manifest.json` — whose per-skill/per-agent entry requirement is the most common way `docs_sync --check` fails a commit, and which a contributor reading the tree had no way to learn about. `docs/` was missing entirely despite holding six hand-maintained deep-dives. The tree also said pre-commit runs "the budget gate" while the Versioning section two paragraphs later said "budget gate + `docs_sync.mjs --check`"; the tree now matches.

  **`docs/GRAPHIFY.md` had never learned about `epic-workflow`** — zero mentions, though `epic-workflow` step 10 proposes `graphify update .` like `task-workflow` step 6 does. It was missing from the skill count, the §4 per-skill list, the mermaid rebuild cluster, and §6's list of rebuild points; all four now include it. The count went **four → five** adapting skills, and `knowledge-distill` is now explicitly named as *not* one of them: it treats structural facts as extracted rather than distilled whether a graph exists or not, which is a boundary, not an adaptation — the distinction the old count blurred.

  **All four manifest `description` strings predated the workflow trio.** Every one described the plugin as harness setup + scaffolding + distillation + session handoff, never mentioning `task-workflow`, `epic-workflow`, or `discovery-workflow` — which README's own prose calls "the main driver" you run "dozens of times". All four now lead with the spec-gated workflow.

  **`docs/SPEC-GATE.md` never mentioned `epic-workflow`.** A reader could finish it without learning that `.claude/memory/EPIC.md` deliberately doesn't satisfy the guard — a load-bearing invariant (`skill-authoring.md` states it, and it was documented only in USER_GUIDE). Stated in place now, next to the other `PLAN.md` field rules.

  **One keyword gap, and one non-finding.** `.claude-plugin/marketplace.json` carried 91 of `plugin.json`'s 92 keywords, missing `conventional-commits` — added. The Cursor list's 45-term gap was **not** drift: `tools/docs_sync.mjs:202` records that keyword lists may diverge and that "the Cursor list is deliberately the shorter, less stack-specific one." An initial pass here synced it to near-parity and was reverted for violating that decision. What was genuinely incoherent is narrower: `mdc` and `agent-hosts` were absent while every one of their Cursor-specific siblings (`cursor`, `cursor-rules`, `cursor-hooks`, `agents-md`, `multi-host`) was present. Those two were added and the stack-specific terms deliberately left out, so the list is now short *and* internally consistent.

  Checked and left alone, so the next sweep doesn't redo it: the 20 bundle-root-relative links a link-checker flags in `knowledge-bundle.md` and friends (`/implementation/index.md`, `/meta/knowledge-bundle-spec.md`) are template paths that resolve inside a target repo's `knowledge/`, not this one; "nine guards" is right (9 guards plus `lib/hook-io.mjs`, which isn't one); `verify-gate.mjs` survives only in CHANGELOG and the audit log, which are historical records and correctly still name it; "all five profiles" in `scaffold-delegation.md` and "three hooks, four scripts" in `GATES.md` §5 both hold; the ≤60-line `CLAUDE.md` claim is consistent across all seven places that assert it; and README's prose plus USER_GUIDE's what-you-get trees are current.
...
- **`write-tests` reads the PRD, so an acceptance criterion becomes an E2E spec without a human retyping it.** The skill had one path: author unit tests for a named file or function. It now has two, routed on what the request names — a file, function, component or module still goes to the seven-step unit path (**unedited**, byte for byte), while `write-tests FR-3/AC-2`, `write-tests FR-3`, or a Given/When/Then criterion quoted inline goes to a new acceptance-criterion → E2E path. With that, all three readers of the `docs/product/prd.md` contract actually read it: `task-workflow`'s full-spec `Covers` column, `epic-workflow`'s step 2 (v1.70.0), and now this one. The row `artifact-formats.md` had been holding open since v1.69.0 — "**No** — a criterion is quoted to it by hand" — is closed, and the three places that stated the old behaviour as fact were corrected with it (`artifact-formats.md`'s contract table and its surrounding prose, `discovery-workflow/SKILL.md` step 5, `docs/USER_GUIDE.md`'s PRD section). A stale negative claim in a contract table is worse than no table: it tells the next reader not to bother trying.

  **Routing plus the essentials in `SKILL.md`, the rules in `references/acceptance-to-e2e.md`** — the repo's usual split, so the invoked-body cost stays near where it was. The reference carries the resolution table (`FR-3/AC-2` → one criterion; bare `FR-3` → every *active* criterion under it; `NFR-n` → the same addressing, with a threshold criterion named as a performance-tier check rather than asserted from a browser driver; an inline criterion → itself, no PRD read), the PRD's own two resolution rules (the `### FR-n` block outranks the index; IDs are permanent, so a numbering gap is never read as an off-by-one), and one refusal table covering every stop case.

  **The PRD stays a fixed, read-only contract.** This path reads `docs/product/prd.md` and never writes it, amends it, or renumbers an ID — not even to fix something obviously wrong, which is reported for `discovery-workflow` to fix upstream instead. Citation style is `epic-workflow` step 2's, deliberately, so the two read alike: address by ID, quote the criterion **verbatim, never paraphrased**, and name the ID in the spec itself so the trace back to the contract survives a file move. Paraphrasing is how a test drifts away from the requirement it claims to cover while both still look green.

  **Two rules are stated where they cannot be skimmed past, because both are the kind that get papered over.** First: **if the repo has no E2E tier, this path produces no file** — say so, name the nearest tier that does exist, and stop. No Playwright, no Cypress, no dependency, no invented `e2e/` directory, no spec written into a tree in the hope someone wires a runner later. Adopting an E2E harness has a CI bill and a maintenance owner attached, and a skill invoked to write one test is the worst possible place for that decision. Second: **never claim green on a spec you did not run.** The unit path's TDD order (step 5) does not transfer intact, because an E2E spec usually cannot be driven red-then-green locally; so the requirement is weaker (attempt the run) and the reporting is stricter (exactly three permitted forms — ran and passed with output, ran and failed with output and what the failure means, or *not run* naming precisely what running it needs). A spec that was not run is never called passing, working, verified or done.

  **"The repo's existing E2E style" always means the target repo's own, discovered.** This plugin prescribes no E2E harness anywhere and installs none, so there is nothing to default to: read the repo's `.claude/rules/testing.md` first (it outranks anything inferred from layout), then the nearest existing spec for structure, naming, selectors, fixtures and assertion style, then the runner config for where the file has to live to run at all. A per-profile table says what each scaffolded profile's `testing.md` actually encodes, checked against those files rather than recalled: `nuxt`'s centralized `tests/` tree with `~~/` imports and `tests/support/` Nitro stubs and `next`'s co-located `src/**/*.test.ts(x)` declare **Vitest tiers only and no E2E tier**, so in those repos an E2E tree either already exists or the no-harness rule fires; `flutter` is the one profile that declares a flow tier — `integration_test/**`, on a real device or simulator, already specified as one flow test per acceptance criterion, which is exactly this path's shape; `go`/`nodejs`/`generic` have no testing rule at all. Repo-level tiering (E2E gated to deploy time rather than pre-commit) stays the target repo's `testing.md` call — this path reports where the spec will run and never rewires a gate.

  **The privacy rule travels with the quote.** A criterion carries the PRD's roles-never-people rule: a real name, address or account appearing in one is fixed **in the PRD**, never laundered into a test fixture and never quietly swapped for a role on the way into the spec while the PRD keeps it. Fixture data comes from the criterion's own terms plus the repo's existing fixture helpers, and nothing else.

  **Dry-run against a fixture rather than asserted.** A scratchpad repo with an approved two-requirement PRD (one active FR with two criteria, one `Status: withdrawn` FR), a `testing.md` declaring `tests/e2e/*.spec.ts` as deploy-time-only, and one existing Playwright spec. Following the reference as written: `FR-1/AC-2` resolved off disk and produced `tests/e2e/approvals-list.spec.ts` in the existing spec's Playwright idiom with the criterion quoted verbatim above the test and the ID in its title; the run was attempted and could not execute (`Cannot find module '@playwright/test'`), so it was reported as **not run**, naming what running it needs — which is the rule above doing its job rather than a claimed green. `FR-2/AC-1` refused on the withdrawn `Status:` line and wrote nothing; `FR-9/AC-1` stopped, naming what was cited against what the file holds, with no criterion reconstructed from a title; and "write tests for the `parseWeek` helper" routed to the unit path untouched, reading no PRD.

  No new gate and no new guard — the path is invoked, not enforced. `description:` grew from 140 to 275 chars, inside the 350 ceiling, and `evals.json` gained three should-trigger cases for the criterion phrasing plus a should-not-trigger case keeping "run the e2e suite" out: this skill authors tests, it does not run suites.

## [1.72.0] - 2026-08-27

### Added

- **`bigin-harness-setup` stops asserting things it hasn't checked, and stops growing when it re-runs.** Two behaviours in one release: Phase 2 now runs every command before writing it down, and a fourth `INSTALL_MODE=verify` (Phase 1b) re-checks an existing `CLAUDE.md` against the repo and *corrects or removes* what no longer holds.

  **Run it before writing it down — every profile, not just `generic`.** Phase 2 wrote a Commands table either from `profile-generic.md`'s detection ladder or straight from a fixed profile template, and neither ever executed anything: a `scripts` entry naming a binary nobody installed, or a template row for a task the repo never defined, went into the always-loaded brief as a confident fact. Phase 2 now executes the **lint, typecheck and test** rows before `CLAUDE.md` is written, one at a time and non-interactively, and a row that cannot run becomes the existing `TODO: <lint|typecheck|test> command` placeholder with the row's own purpose word in the slot (`TODO: test command`) — the notation `profile-generic.md` already uses to record an undetected command, and that Phase 4 keeps for `generic` when it fills `AI_REVIEW_CHECKLIST.md`. No new notation was invented. The literal keeps its two pre-existing homes, unchanged in both — `profile-generic.md`'s detection ladder (whose surrounding prose this release does extend) and Phase 4's `AI_REVIEW_CHECKLIST.md` step (untouched), each describing the install-time detection path — while everything this release adds carries no copy of its own: `verify-mode.md` states the string once, under `## The TODO: placeholder`, and Phase 1b and Phase 2's rewrite path both point there. Nothing outside the lint/typecheck/test rows can ever reach that slot, because no other row is executed and an unrun row is never rewritten. `dev`, `build`, `format`, `start`, `watch` and `deploy` rows are never executed and are written as the template has them, reported as unchecked; `{STACK}` and every header line naming a runtime, package manager or framework is read from the manifests and claims only what a manifest actually says.

  **The distinction that decides every rewrite, stated in the reference in full.** A command that **cannot run** (no such script, target or task; binary not on `PATH`) is a stale claim and gets rewritten. A command that **runs and fails** (red tests, lint errors) is the repo's current state, not a false claim — it is reported and the row is kept. Conflating the two has the harness delete a repo's test command because the suite is red, so the reference also says to read the failure rather than the exit code: the behavioural test below produced exit 1 for a missing script, which a status-code-only check would have filed as a red suite.

  **Shrink, don't append.** `verify` writes no harness file at all — no rules, no guards, and `.claude/harness-version` is deliberately left alone, because no version changed: nothing was installed. The only file it may edit is `CLAUDE.md`, and every fix is made in place: the stale line is rewritten, the stale row removed. **A verify pass may shrink `CLAUDE.md` or leave it the same size; a verify pass that grows it is a bug** — `references/verify-mode.md` says exactly that, in those words, because "append a clarification beside the stale line" is the failure mode every agent reaches for first. It fails loudly by design: a recorded command that no longer runs leads the summary rather than being buried in a list of successes, and every correction and removal is quoted with what it used to say, so a wrong removal is visible in one read instead of found later in a diff. The summary prints one bucket per outcome — broken first, then failing-but-kept, verified, corrected, removed, could-not-verify, plus a gaps bucket for rows that were already `TODO:` — and omits the empty ones.

  **Only a claim verify actually checked may be changed.** A header clause **contradicted** by a manifest is corrected or removed; one merely **unsupported** — `Node ≥22` where nothing records an `engines` field — is could-not-verify and is left exactly as it is. Deleting a claim you couldn't check isn't shrinking, it's guessing. Same rule for a `CLAUDE.md` in no recognizable shape: verify checks claims, not layout, so unparseable rows are reported and left untouched — reformatting someone's hand-edited file is not a stale-claim fix. A `TODO:` row already present is reported as a gap, never as "verified".

  **Verify mode executes commands read out of a file in the target repo**, which is the one genuinely new risk here, so the bound is written down rather than assumed. `CLAUDE.md` is committed and reviewed, so this is not attacker-controlled input in the usual sense — but only the commands recorded in that file's Commands table are run, exactly as recorded, one at a time, never chained, never assembled from anything else in the file, never a command the table doesn't hold, never with added flags to make an unrunnable command run, and never through a shell that would re-interpret a row's own text — a recorded command carrying `$(…)`, backticks, `&&`, `;`, a pipe or a redirect is reported rather than run, since what would execute is no longer the claim under test. Only lint/typecheck/test rows are ever executed, judged by the command rather than the `Purpose` label, and anything that waits for input, runs forever, rewrites the tree or touches a remote is reported as could-not-verify instead: the harness must not start a dev server to check a table row.

  **Tested on a fixture rather than asserted.** A small `package.json` repo with real `lint`/`typecheck`/`test` scripts and a generated-shape `CLAUDE.md`, then deliberately drifted — `test` renamed to `test:unit` out from under the recorded command, and the directory the `Stack:` line named removed. Following `verify-mode.md` as written: the broken `test` row was rewritten to `TODO: test command` and named first in the summary, the dead entry-point clause was removed from the `Stack:` line, `lint` and `typecheck` verified clean, `npm run dev` was never executed, and the file went from 1051 to 1023 bytes across the same 24 lines. Running the procedure is also what found the unsupported-vs-contradicted gap above, which was fixed in the reference and the pass re-run.

  **One pre-existing instance of the same disease, fixed with it:** `references/decision-bundle.md`'s install-mode question still read "the overwrite/new/cancel question" and had been missing `patch` since patch mode shipped in v1.22.11. It now names all five answers and says that two of them — `patch` at Phase 1a, `verify` at Phase 1b — end the run in their own self-contained phase. No new guard and no commit-time gate: verify mode is invoked, not enforced.
...
- **`epic-workflow` now reads a PRD, when one is on disk and approved.** v1.69.0 fixed the PRD's format on the promise that `epic-workflow` would eventually parse it; this release is that wiring. Step 2 was "up to 3 questions about the decomposition" unconditionally — now it checks `docs/product/prd.md` first, and when an approved PRD exists and covers the initiative, it asks **zero** questions and decomposes straight from the contract instead. The no-PRD path is untouched, byte for byte: the exact "Up to 3 questions…" sentence survives as the fallback branch, taken whenever there's no PRD, a draft one, or one that doesn't cover the initiative at hand.

  **The mapping is fixed, not improvised**, because guessing at it defeats the point of having a contract: the requirement index becomes the unit candidates (one per FR, or one per tight FR cluster sharing a surface — never one unit spanning two unrelated FRs); `Depends on:` becomes `Blocked by`, and is the only legitimate source of one when decomposing from a PRD; `Surface:` feeds the two-plus-surfaces half of step 1's own triage bar; `Priority:` decides what the first epic-sized slice contains when the PRD blows past the existing ~8-unit ceiling, `must` first; and each FR's acceptance criteria become that unit's `Acceptance` cell, **quoted, never paraphrased**, so a unit's own later spec gate is audited against the PRD's actual words rather than someone's summary of them. Units cite the FR IDs they cover, so the decomposition stays traceable back to the contract, and any active requirement no unit covers gets named out loud as deferred rather than silently dropped.

  **A PRD doesn't get a pass on the existing rules — it has to survive all of them.** Step 3's four decomposition rules and the ~8-unit ceiling govern a PRD-derived decomposition exactly as they govern any other. Named edge cases cover the new ways a PRD introduces to get this wrong: a `draft` PRD isn't a decomposition source (report it, offer `discovery-workflow` or the fallback questions); no `Status:` line reads as approved, matching `discovery-workflow`'s own rule; a `Status: withdrawn` requirement never becomes a unit or a `Blocked by`; a PRD that exists but doesn't cover the initiative falls through to the questions rather than being force-fit; more than ~8 units still hits the existing ceiling; a stale index against an authoritative FR block still loses to the block; an FR whose acceptance criteria can't hold without a later unit is split or reordered rather than forced into one unit; and resuming an in-flight `EPIC.md` never re-reads a PRD to re-decompose an already-approved queue.

  **Read-only, always — stated explicitly rather than left implicit.** `epic-workflow` never writes or amends `docs/product/prd.md`, at any step; the only thing that changes about it during an epic is which requirements are `Covers`-cited from `.claude/memory/EPIC.md`. That queue file gained two optional, PRD-derived-only fields for exactly that purpose: a `PRD:` header line naming the source, and a `Covers` column per unit listing the FR IDs it cites. Both are omitted entirely for an epic with no PRD — no placeholder, no padding.

  **This makes several claims elsewhere in the plugin false, and all of them got fixed in the same release rather than left to drift.** `discovery-workflow`'s step 5 used to say `epic-workflow` and `write-tests` "do not parse a PRD yet — neither skill mentions one"; its step 7 handoff said `epic-workflow` "does not read the file itself, so pointing at these is the handoff, not a courtesy"; its "Interaction with the skills downstream" section said "nothing in `epic-workflow` reads the file." `artifact-formats.md`'s reader table had `epic-workflow` down as "No." `USER_GUIDE.md` repeated the same claim in prose and in its epic-workflow flow diagram's "≤3 questions, decomposition-level only" line. All of them now say what's actually true: `epic-workflow` reads the file itself, conditionally, at its own step 2; `write-tests` is the one still unwired, and stays that way — that's a later epic unit, deliberately out of scope here, same as the PRD format itself, which this release leaves untouched as fixed contract.

## [1.69.0] - 2026-08-21

### Added

- **`discovery-workflow` — the layer above `epic-workflow`.** Everything in this plugin started at "we know what to build." `epic-workflow` step 1 takes an initiative as given, `task-workflow` step 1 takes a task as given, and both are right to — but nothing produced what they assume. A vague client ask ("we want a client portal") had no path through the harness at all: the brief lived in someone's head, the requirements were re-derived at every spec gate, and the architecture decisions the product forced were written down retroactively at best. The new skill owns exactly that gap and stops at the PRD.

  **Triage first, because most asks aren't discoveries.** Three rungs, stated once: a one-line change, copy fix, config tweak, or bug goes to `task-workflow` (or `debug-workflow`); a named initiative whose product shape is already settled goes to `epic-workflow`; only a vague ask, a new product surface, or an initiative nobody can write acceptance criteria for continues here. The discriminator between rungs 2 and 3 is a test rather than a feeling: *can you write one testable acceptance criterion for the request exactly as stated, inventing nothing?* If yes, the product question is answered. **Rungs 1 and 2 write nothing to disk** — no stub brief, no empty `docs/product/`, no placeholder concept — because an artifact that exists because a skill was invoked, rather than because someone needed it, is the paperwork the ladder exists to avoid.

  **Two homes, and nothing crosses.** Product artifacts are human-facing durable docs — `docs/product/brief.md` and `docs/product/prd.md`. Architecture decisions are agent-facing OKF v0.2 concepts under `knowledge/architecture/`. A requirement never lands in `knowledge/`, an invariant never lands in the PRD, and neither file restates the other. The distinction is the reader, not the subject: "a user can only see their own workspace's invoices" is a promise someone approves and later checks; "every row belongs to one workspace and every read filters by it" is a constraint on every future change that nobody approves and everybody must obey. The paths are fixed rather than configurable — downstream skills read a path, they don't search for one.

  **The PRD is a contract, which is why its format isn't stylistic.** Requirements are `### FR-n — {title}` blocks with a fixed field set (`Requirement:`, `Priority:`, `Surface:`, `Depends on:`, `Status:`, `Acceptance criteria:`), each carrying at least one numbered Given/When/Then criterion that is observable from outside the system. `epic-workflow` can decompose from the requirement index and the `Depends on:` lines; a test generator can read one criterion at a time, cited as `FR-3/AC-2`; a `PLAN.md` cites the ID in its `Covers` column. Only the last of those is wired today — the full-spec `Covers` column already takes requirement IDs, while neither `epic-workflow` nor `write-tests` mentions a PRD, and this release doesn't change either. The format is fixed ahead of them on purpose: all three cite by ID, so **IDs are assigned once, never renumbered, never reused** — a tidy-up that closes a gap left by a withdrawal silently re-points every existing citation at a requirement nobody wrote. Withdrawn requirements keep their number and say so. The blocks are authoritative and the index is navigation, stated explicitly because two lists of the same IDs is a drift risk worth naming.

  **Structured elicitation, with a cap that has a defined behavior when it's hit.** The harness's usual "up to 3 clarifying questions" is the right budget for a task whose shape is known and the wrong one for a product question in both directions — too small to establish who a product is for, too open-ended to stop an agent asking twelve variations of one thing. `references/elicitation.md` replaces it with seven named techniques routed by which brief section is missing (concrete instance, problem inversion, role census, non-goals first, failure interview, volume and horizon, disagreement probe) and a hard cap of 3 rounds / 4 questions per round / 12 questions total. **At the cap it stops asking and writes the brief anyway**, with every unresolved item as an `## Open questions` row carrying a deciding role and a working assumption. That's the better artifact: it makes the remaining uncertainty visible at the approval gate, where the person who can actually resolve it is looking, instead of optimising for the agent's own confidence at the user's expense.

  **Established-repo mode reads before it asks, and runs before it writes down.** A repo counts as established if it carries application source, project documentation, **or** a `knowledge/` bundle — any one of the three, because a bundle-and-markdown repo has no source files and is still one of the richest reads available. A read order from `docs/product/` (the resume check) through `CLAUDE.md`, `knowledge/index.md`, the surface inventory, and the test suite — which is the most reliable written statement of intended behaviour in any repo, because it's the only one that fails when it's wrong. A `Don't ask / Read instead` table covers the seven questions that lose a user's confidence in the first five minutes. **Every command that appears in the brief was executed in the session that wrote it**, with its verdict recorded; a command that fails is recorded as failing, because a documented command that no longer runs is a claim the repo has been making to every agent and every new hire since it broke. Inspection is read-only, per an explicit allow/never table — build caches and coverage output are acceptable side effects, a tracked file, a database, or a remote is not, and if the only way to verify a claim is a mutating command the claim is recorded unverified instead.

  **Two approval gates, no new commit-time guard.** The brief and the PRD are each presented in chat and written only once approved: an unapproved artifact on disk is indistinguishable from an approved one on the next session's resume. **Nothing enforces either gate mechanically, and the skill says so in step 4 rather than implying otherwise.** `disallowed-tools` withholds `Edit`, `Write` and `NotebookEdit` for the invoking turn, which is a courtesy and not a guarantee — a shell redirect writes a file just as well, and Bash has to stay available because established-repo mode runs commands. Behavioural testing confirmed exactly that: the triage exit wrote nothing because the instruction said not to, not because the frontmatter key stopped it. The discipline is the mechanism here, so the rule is stated plainly and the absence of a backstop is stated with it. Nothing at commit time reads a brief or a PRD, deliberately: a PRD gate would fire in every repo that installs the harness, and most of them will never have one. Existing gates already pass this work unchanged, since `spec-gate-guard.mjs` treats every `.md` path as trivial.

  **Resume never overwrites an approved artifact.** `Status: approved` is write protection and `Status: draft` is not — that's the whole purpose of the line, and no guard reads it. A file with no `Status:` line was hand-written by a human and counts as approved, because the safe direction is to never overwrite what you can't prove is a draft. Re-invoking over an approved PRD reports state and goes straight to handoff; changing one is an amendment (additive with the next unused number, withdrawn in place, changed in place, or — when the premise itself moved — a new brief that supersedes rather than a PRD patched past recognition).

  **Architecture decisions land as OKF v0.2 concepts, written ahead of the code.** `type: Contract` for an invariant (could a future change break this without anyone noticing?), `type: Domain` for a shape decision — the two a discovery can honestly claim in any version of the allowed list, since the rest need something already running to describe. `references/architecture-concepts.md` tabulates the frontmatter against what `knowledge_validate.mjs` errors on versus warns on **where that script is installed**, and says what to do where it isn't: bundles predating the v0.2 move declare themselves v0.1-inspired with a longer `type` list and no trust keys, so the bundle's own `meta/knowledge-bundle-spec.md` and its existing concept files win over the reference. It requires the `knowledge/index.md` line — copying the bullet and dash style already in that file, and touching nothing else in it, including whatever frontmatter it carries — and names the link trap that would otherwise fail a repo's pre-commit: bundle-relative links resolve against `knowledge/`, so `[PRD](/docs/product/prd.md)` is a hard error and the PRD is referenced through `sources:` instead. Concepts land `status: draft` with **no `verified` entry** — they describe a system that doesn't exist yet, absence of `verified` is how the bundle says exactly that, and the first task that implements the decision is what earns `stable`. Expect one to three concepts per discovery; more than five means the PRD is being transcribed into context every later task pays for.

  **It skips cleanly with no bundle.** No `knowledge/` directory means the step is skipped out loud — naming the decisions it would have written and where they would have gone — and no directory is created. Half a bundle is worse than none: unvalidated frontmatter, no pre-commit gate, unreachable from any index, and a file that looks authoritative while nothing has ever checked it.

  `epic-workflow`, `task-workflow` and `write-tests` are untouched by this release — each is handed the PRD path, or one requirement ID, rather than discovering the file itself, and a repo with no PRD keeps working exactly as before. The format is fixed now so that reading it stays the same job whichever way the path arrives.
...
- **`bigin-harness-setup` now has a `generic` profile, and stops asking which of four stacks an unmatched repo is.** Phase 0's four-way question was reachable in two very different situations: an empty repo, where the answer picks the scaffold and is genuinely needed, and an existing repo whose stack simply isn't nuxt/go/nodejs/next — where every available answer is wrong and picking one writes conventions, `paths:` globs, permissions, and a pre-commit script for a stack that isn't there. The question is now asked **only** for an empty repo; existing code with no marker match sets `PROFILE = generic`, says so in one line, and continues.
  - **What generic installs**: `CLAUDE.md` (a `{STACK}` line plus detected commands, with a "match the surrounding code" rule standing in for the missing conventions file), `.claude/rules/security.md` + `architecture.md`, the AI files, all eight guards, `.claude/settings.json`, `tools/context_budget.mjs`, `scripts/pre-commit.sh`, and every opt-in phase (knowledge bundle, graphify, model routing) unchanged.
  - **What it skips, and why each one**: Phase 0.5 scaffold (no scaffold skill for an unknown stack, and generic is only ever reached from a repo that already has code); `conventions*.md` / `testing.md` (no stack to write conventions for — the existing code is the spec); `.vscode/settings.json` (formatter unknown); Phase 5.6 CI (`CI_PROVIDER` is forced to `no` and the question is omitted — a workflow is only as good as the commands, runtime, and services in it, none of which are inferable here). The Phase 7 summary names the skips and lists the commands a hand-written CI job should run.
  - **Commands are detected, never guessed.** In order: `package.json` scripts (runner from `packageManager` or the present lockfile), `Makefile` targets, `justfile`/`Taskfile.yml`, then language-conventional defaults where the manifest exists (`pyproject.toml`, `Cargo.toml`, `composer.json`, `Gemfile`, `pom.xml`/`build.gradle`). Anything undetected stays the literal `TODO: <lint|typecheck|test> command` in `AI_REVIEW_CHECKLIST.md`, degrades to a no-op `echo` in `scripts/pre-commit.sh` (green gate, visible gap), and is dropped from `CLAUDE.md`'s Commands table — a table of placeholders is noise, and the summary already reports it.
  - **`permissions.allow` is git-only** for this profile. An unknown toolchain gets no blanket allowlist; the user approves its commands as they come up.
  - New `references/profile-generic.md` (what it installs/skips, command detection, `CLAUDE.md` + `settings.json` templates, why no CI), a `generic` entry in `references/files-shared.md` → `## paths substitutions` (scoped by source-file extension plus whichever contract file is actually present, since there's no directory convention to scope to), and `## pre-commit: generic` in `references/hook-guard.md`. No `patch` block — an already-scaffolded repo has a real profile, so nothing here applies to it.
...
- `README.md`'s title and tagline rewritten (`# bigin-skills` → "# BigIn Skills") to lead with `task-workflow`/independent-verifier/enforcement-gates, matching the repo's GitHub description, instead of the generic "collection of skills" framing.
...
- **`oss-port` gained an explicit spec-authoring step and a multi-session sprint plan, replacing "the source is the spec" with written, gated artifacts.** Previously the port's only truth artifacts were `PORT/FEATURES.md` (an inventory checklist) and `PORT/contract/` (interface-only) — no behavioral spec existed, "module" was improvised at the old Phase 6 from FEATURES.md's category-organized rows, and multi-session progress hung entirely off FEATURES.md checkboxes. Phase 3 is now **Spec & contract extraction**: a new 3a groups FEATURES.md's CORE/ADAPT rows into named modules (recorded in a new `Module` column on the FEATURES.md template) and writes `PORT/spec/<module>.md` per module from a new template in `references/templates.md` — derived by reading the reference source, stack-neutral except a Target-adaptations section, quoting exact thresholds/conditions with `reference/` file:line; depth scales with module complexity and small ports (≤~5 modules) may combine closely related modules into one file, but specs are never skipped. The old contract bullets became 3b unchanged, plus a cross-check that every contract surface maps to a module spec and vice versa; one gate approves both. Upfront specs are the default, with a documented opt-out for >~10-module ports (approve contract + module list + sprint-1 specs now, defer later sprints' specs to their sprint boundary, recorded as "write + approve specs" rows in the plan). New **Phase 4 — Port plan** produces `PORT/PLAN.md` (second new template): sprints in dependency order, each a roughly session-sized batch of modules with an explicit gate, plus a module table linking each module to its spec and sprint; FEATURES.md stays the fine-grained row-level ledger (a module is Done only when all its rows are checked), and the analysis-only fence moved from "Phases 0–3" to "Phases 0–4". Old Phases 4–7 renumbered to 5–8 (clean renumber, no sub-phases — cross-references updated in `idiom-translation.md` and `graph-index.md` too, CHANGELOG history left as-is). "Resuming a port" now reads `PORT/PLAN.md` first (which sprint is in flight, which module is next), FEATURES.md second for row-level state, and offers ports started under older skill versions (FEATURES.md present, no PLAN.md) a choice between backfilling the Phase 3–4 artifacts or continuing on checkboxes alone. Phase 7's subagent brief now includes the module's spec in full (spec says what the behavior must be; source stays ground truth for exact mechanics), the independent read-only verifier checks the diff against the spec as well as FEATURES.md rows and PATTERNS.md, and step 9 updates PLAN.md statuses, stops at each sprint gate, and at sprint boundaries proposes `session-handoff` (and `sprint-distill` where the target repo has a knowledge bundle). Phase 8's parity report additionally confirms every sprint is Done. New anti-pattern: porting a module with no approved spec. `session-handoff`'s mid-workflow-saves list now names `oss-port` with `PORT/PLAN.md` + FEATURES.md checkboxes as the disk truth re-derived on resume. No ` ```patch ` block — nothing here is copied verbatim into target repos.
