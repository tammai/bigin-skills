---
name: go-scaffold
description: "Scaffolds a production-ready Go modular-monolith REST API — non-interactive, template-driven. Contract-first: api/openapi.yaml (tagged per module) generates each module's chi-server interface + models (oapi-codegen), each module's queries/*.sql generates typed queries (sqlc); the script runs both generators itself so the repo it leaves behind builds and tests green, not a skeleton needing manual fixup. Ships two example modules — users (JWT+argon2id auth, RBAC, rotating refresh tokens) and posts (a cross-module read dependency, optimistic-concurrency updates) — each behind its own /v1/<module> route prefix with compiler-enforced module boundaries (Go's nested internal/), plus soft-delete erasure with a synchronous cross-module anonymize. MUST use when user says: 'scaffold go api', 'create go rest api', 'new go backend', 'initialize go api', 'go rest api scaffold', 'set up go api', 'tạo go api', 'khởi tạo go api', 'cài go api', or when the repo has no go.mod. Also invoked by bigin-harness-setup Phase 0.5 for the go profile on an empty repo."
effort: low
