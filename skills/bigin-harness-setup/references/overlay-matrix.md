...
**`bugfix-test-guard` off in `specs` and `contracts`.** It needs a test surface. `specs` has none; `contracts` has one only in principle (`oasdiff`, a separate workstream), and the guard's trivial-path allowlist does not cover `.yaml` — so every contract fix would block on a regression test that cannot exist. It stays on in `qa`, where a fix to an E2E spec *is* a test file. The general rule this instance of: `.claude/rules/skill-authoring.md` → a gate goes where its premise holds.
