...
What generic **does** install: `CLAUDE.md`, `.claude/rules/security.md` + `architecture.md` + `comments.md`, the AI files, every guard wired in the `settings.json` block below (all nine), `settings.json` itself, the context-budget gate, the pre-commit script, and every opt-in phase (knowledge bundle, graphify, model routing).
