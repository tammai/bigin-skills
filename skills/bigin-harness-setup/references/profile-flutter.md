...
**The `api/generated/**` line below is necessary and not sufficient.** The dart-dio generator emits a *standalone package* with its own `analysis_options.yaml`, and the analyzer resolves options from the file nearest each source — so this root exclude never reaches those files, and `--fatal-infos` fails on generator noise anyway. `tool/generate_api.sh` therefore writes that package's options file too. Keep the line here regardless: it covers the `*.g.dart` output the app itself generates.
...
The app depends on the result by path — add it once, in `pubspec.yaml`:
