...
2. `nuxt.config.ts` or `nuxt.config.js` **plus two more conditions** → profile = `nuxt-marketing`. **This rung is above `nuxt` for the same first-match-wins reason row 1 is.** A multi-locale marketing site carries the `nuxt.config.ts` marker too, and `nuxt` matching first would write BFF-proxy, sealed-session and Pinia-Colada conventions into a repo with no server half, no auth and no client state worth a store — and would have no way to say the thing that actually matters here, which is that a client's content editor may change content files and locale bundles and nothing else. All three conditions are required:
   ...
   - **No BFF or auth marker** — no `server/api/**` directory, and no `nuxt-auth-utils` in `dependencies`. This is the narrowing test, and it is the one that keeps row 3 untouched: a Nuxt fullstack app that happens to ship a docs section satisfies the first two conditions and **must** stay on `nuxt`.

   The third condition holds for the life of a real marketing site rather than only at install time, because this profile's two server endpoints — a contact form and a newsletter signup — live in a Worker of their own, not in this app's `server/api/`. A site that grows a `server/api/**` route or takes on `nuxt-auth-utils` has stopped being this profile; that is a re-run of setup with the change made deliberately, never a silent reclassification.
