...
Detection: a **new rung above `nuxt`**, matched only when all three conditions hold — `nuxt.config.ts`/`.js` present, `@nuxt/content` in `dependencies`, and no BFF or auth marker (`server/api/**` absent, `nuxt-auth-utils` not in `dependencies`). First match wins in the Phase 0 ladder, so this rung has to sit above `nuxt` for the same reason `tauri` does: a marketing site carries the `nuxt.config.ts` marker too, and `nuxt` matching first would write BFF-proxy and Pinia-Colada conventions for a repo that has no server half and no client state worth a store. Condition 3 is the narrowing test and it points the other way as well — a Nuxt fullstack app that happens to ship a docs section satisfies conditions 1 and 2 and must stay on `nuxt`. Full ladder: `references/profile-detection.md`.

**Empty repo → this profile is never reached.** There is no scaffolder for a marketing site here, so the empty-repo question in `references/profile-detection.md` is deliberately *not* extended to it: offering a choice this skill cannot scaffold is the failure mode `references/profile-generic.md` names for Flutter packages. The repo this profile onboards has already been scaffolded by the Marketing Site Factory's template, and Phase 0.5 is skipped for it the same way it is for any repo whose marker file already exists.
...
There is no `server/api/`, no session, no store of server state and no client-side data fetching against a remote origin. Every page is prerendered at build time from a content collection. Every Nuxt answer that reaches for `server/api/`, `nuxt-auth-utils` or a Pinia Colada query is wrong in this repo — and wrong in a way that passes `pnpm dev`, because the dev server has a Nitro half that the deployed assets do not.
...
**2. Worker routes are the only server surface, and only for forms and newsletter.**
The site itself is fully prerendered static assets: there is no request-time server for a page. The two exceptions are a contact form and a newsletter signup, and they live in a Worker of their own — not in this app's `server/api/`, which stays absent. Three things are requirements on those routes, not suggestions: **Turnstile verification** of every submission, **rate limiting** per IP and per form, and **payload validation** against a declared schema before anything is read. A form endpoint with two of the three is an open relay for whoever finds it.
