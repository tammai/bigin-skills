...
**`nuxt.config.ts`** — insert `runtimeConfig: { backendUrl: '' }` at the position `nuxt/nuxt-config-keys-order` demands: `insertRuntimeConfig()` scans top-level keys in file order and inserts before the first key that rule ranks *after* `runtimeConfig` (the comment goes on its own line — a trailing comment trips `@stylistic/no-multi-spaces`). It does **not** anchor on `routeRules`: 5 of the 8 cloned templates (`landing`, `docs`, `portfolio`, `chat`, `editor`) ship none, and `css` is the wrong anchor too — the client-module keys `content`/`mdc`/`ui` sit between `css` and `runtimeConfig` in the rule's order. `KEYS_BEFORE_RUNTIME_CONFIG` in `scaffold.mjs` mirrors that rule's `ORDER_KEYS` prefix; keys the rule doesn't know (`eslint`, `ogImage`, `llms`, `mcp`) sort last, so they count as "after". Where a template already has a `runtimeConfig` (`editor`: `public.partykitHost`), `backendUrl` is merged into it — a plain `includes('runtimeConfig')` skip would leave the proxy with no backend URL. `pnpm lint` at Stage 5 runs without `--fix`, so a misplacement fails the scaffold instead of self-healing. Do **not** add `compatibilityVersion: 4` (stale Nuxt 3→4 migration flag). The `ui` template ships this file *without* a trailing newline — `@stylistic/eol-last` fails lint unless fixed; the script appends one. `'@pinia/colada-nuxt'` is registered into `modules` by Stage 2 (`ensureModuleRegistered`) — **required**, not optional; per the [official Nuxt guide](https://pinia-colada.esm.dev/nuxt.html) `useQuery`/`useMutation` don't work without it, and the module auto-installs `PiniaColadaSSRNoGc` for SSR-safe caching with no extra `await`. The template ships `devtools: { enabled: true }` — the script flips it to `enabled: false` in place (BFF preset convention: devtools off by default); fails loudly if that literal isn't found rather than guessing an insertion point.
...
- **`shared/api-client/index.ts`** + **`schema.d.ts`** — `apiClient`, a `$fetch` instance (Nuxt's global ofetch — no HTTP-client dependency) bound to the same-origin `/api/backend` proxy, plus the `Ok<Path>` helper that pulls a 200 response body out of the generated `paths`. `schema.d.ts` is generated from `openapi.yaml` by `pnpm openapi-types` (do not hand-edit).
