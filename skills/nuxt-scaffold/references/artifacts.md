...
- **`tests/app/composables/queries/users.test.ts`** — validates the whole Vitest + Nuxt env + Pinia Colada chain (fresh query is `'pending'` — Colada has no `'idle'`, `useQuery` fires eagerly). This is the *only* test file the base preset ships — without it `pnpm test` fails with "no test files found", so don't drop it without adding a replacement. Lives under `tests/`, mirroring `app/`, and imports the source via the `~~/` root alias — per the centralized-tests convention in `.claude/rules/testing.md` (added by `bigin-harness-setup`).
