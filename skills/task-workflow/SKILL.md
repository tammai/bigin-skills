...
4. **Implement/verify loop** — an independent verifier subagent, not just tests, checks every diff against `PLAN.md` before it reaches review. Skip the whole loop (implement inline, then just run lint+typecheck+tests yourself) only when the spec gate itself was skipped **and** `model-router`'s verification bar came back "normal gates" — a trivial-looking change that touches a contract, migration, or CI path still goes through the loop. The loop guards against spec drift on non-trivial work; it isn't there to gate copy fixes.
   ...
   2. **Dispatch a fresh verifier.** Call the Agent tool with `subagent_type: "bigin-skills:" + routing.agents.verifier` and `model:` set to `routing.models.verifier`, both from `model-router`'s signals (or `bigin-skills:verifier` on its own default if scoring was skipped) — read the agent off `routing` rather than assuming, the same as the implementer above; every profile currently resolves the verifier to `verifier`, and a profile that pinned it lower would resolve to `verifier-medium` without this step noticing. Pass `PLAN.md`'s path and **the diff itself** — never the implementer's own summary of what it did, that's exactly what independence is for. Parse the response against `references/verify-contract.md`'s schema.
   3. **On `FAIL`** — resume the *same* spawned implementer agent with `SendMessage` (`to:` its agent ID) relaying the issues list verbatim, so it fixes only what was flagged instead of re-deriving the task. Then dispatch a **new** verifier (fresh Agent call, new agent ID, no memory of this round) against the new diff. Note the round in `PLAN.md`'s task-row `Notes` (e.g. "Fix-loop round 2/3"). Cap at 3 rounds. Don't re-run model-router's scoring on a resume — the task's underlying complexity doesn't change round to round, only the issue list does; if the implementer itself decides the tier was wrong, it uses the normal `ROUTING_MISMATCH:` handback instead.
      ...
      **Trivial-fix carve-out.** A resume costs the implementer a full context rebuild, which is a bad trade for a two-word correction. So the orchestrator may apply the fix itself — but only when **every** issue on the list clears all four bars:

      - **The issue already names the correct value**, so applying it takes no decision: a typo, a wrong path/name/number/version, a stale cross-reference, a wording fix. If two valid fixes exist and you'd have to pick one, it isn't trivial.
      - **It's text, not behaviour.** No logic, control flow, contract, schema, config value, or test assertion moves. A test's *name* is text; what it asserts is not.
      - **It's bounded** — one hunk, a couple of lines at most, in a file the diff already touches. Never a new file.
      - **All-or-nothing.** One issue that fails any bar sends the *whole* list back to the implementer. Never split a list — fixing the easy ones yourself and relaying the rest leaves the implementer re-deriving against a diff it didn't write.

      Nothing else about the round changes. **Dispatch a fresh verifier** against the new diff exactly as above — the independence that matters is in the audit, not in who typed the fix, and a self-applied fix is the case where an unaudited diff is easiest to talk yourself into. The round still counts against the cap of 3, and `Notes` still records it, marked for what it was (e.g. "Fix-loop round 2/3, orchestrator-applied"). If an orchestrator-applied round comes back `FAIL`, the next round goes to the implementer — never self-fix twice running on the same issue.
...
6. **Cleanup** — once every task in `PLAN.md` is `Done` and review is resolved (clean, or explicitly declined by the user), archive `PLAN.md` out of the repo root. It stops being a working file for the task, but it doesn't stop being the only written record of *why* the task took this shape — so it moves rather than being deleted. Close out any mirrored tasks in the same pass, so a finished task doesn't leave a stale list behind.
   ...
   **Then archive it**, to exactly one of two destinations — never both:
   ...
   - **The repo has `knowledge/implementation/`** — write `knowledge/implementation/{YYYY-MM-DD}-{slug}.md` from the `Record` template in `${CLAUDE_PLUGIN_ROOT}/skills/bigin-harness-setup/references/knowledge-bundle.md`: `type: Record`, `source: plan`, `shipped:` the version this landed in, and a body that is the `PLAN.md` **verbatim** — spec, tasks table with its `Notes`, and the `## Amendments` section if there is one. Append one line to `knowledge/implementation/index.md`, newest first. Then delete `PLAN.md`.
