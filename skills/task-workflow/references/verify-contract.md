...
Single source of truth for the verifier's output schema in `task-workflow`'s implement/verify loop. Referenced by `agents/verifier.md` and `agents/verifier-medium.md` (identical bodies) as their system prompt and by `SKILL.md`'s orchestration step that parses the result — don't duplicate this schema elsewhere; point at this file instead.
