...
`opus-centric` is the only ladder that routes the standard tier below `high`. Every profile now runs the verifier at `high`: the three differ on *model*, and on the standard tier's effort, and nowhere else.
...
| `lean` | Cost-first, and it trades the *other* way: a cheaper model at fuller effort. The standard tier drops opus→sonnet but keeps `high`, buying back with thoroughness what it gives up in capability. The saving comes off the *model* axis only — the verifier stays at `high` like every other profile, because the one agent whose failures are invisible is the wrong place to economise. Deep still escalates to `opus` — the tier where a cheaper model actually costs you. |
...
`verifier-medium` is therefore spawned by no profile. It stays in `agents/` and in `AGENTS` as the mechanism a future profile would use to pin the verifier down; nothing reaches it today, and effort is not settable in `.claude/model-routing.json`, so nothing in a project can reach it either.
...
The pins below are the **default (`opus-centric`) ladder**. The other two deviate in one place each, both documented in the profile table above: `frontier` and `lean` raise standard to `high` — `frontier` paying up front instead of per verifier round, `lean` buying back with effort what a cheaper model gives up. Only `opus-centric` runs the standard tier at `medium`, so that argument below is specifically *its* argument, not a property of the plugin. The verifier argument is the plugin's, on every profile.
...
**`lean` used to be the exception here**, running its verifier at `sonnet`/`medium` on the argument that it is the single most-run agent in the loop and so the profile's largest line item. That bet has been withdrawn: the saving was real, but it was taken against the one agent whose failure mode is silent, and everything argued directly above is a reason not to. `lean` now economises on the model axis alone, which is the axis whose failures a later round can still catch.

A project that wants more than `sonnet` on the verifier still buys the model back per tier:

```json
{ "profile": "lean", "models": { "verifier": "opus" } }
```

(Model only — effort isn't settable in this file, and at `high` on every profile there is nothing left there to set.)
