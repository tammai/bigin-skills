...
This is the default tier for `task-workflow`-driven work: scope → (spec gate if non-trivial) → implement → verify → review. Follow that flow and the repo's `.claude/rules/` conventions. For bug fixes, use the `debug-workflow` skill's four-phase process rather than ad-hoc trial and error. For new test files, follow the `write-tests` skill's discipline.
