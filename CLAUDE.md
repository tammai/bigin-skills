...
- **Most** `references/*.md` under `bigin-harness-setup` template content written **verbatim** into target repos — `hook-guard.md` (the guards + commit scripts), `knowledge-bundle.md`, `profile-*.md`, `ci.md`, `cursor-parity.md`, `graph.md`, `budget-gate.md`, `files-shared.md`. Seven are the skill's own procedure and are never copied: `patch-mode.md`, `verify-mode.md`, `profile-detection.md`, `decision-bundle.md`, `rule-files.md`, `scaffold-delegation.md`, `summary-checklist.md`. Changing a templated one wants a CHANGELOG `patch` block, or already-scaffolded repos only get the change on a full re-run.
