...
scripts/git-hooks/        ← pre-commit running the budget gate + docs_sync.mjs --check
```
...
- Three reference families under `bigin-harness-setup` are copied **verbatim** into target repos — `files-shared.md`, `knowledge-bundle.md`, `profile-*.md`. The rest (`patch-mode.md`, `verify-mode.md`, `profile-detection.md`, `decision-bundle.md`, `summary-checklist.md`) are this skill's own procedure and are never copied. Changing a copied one wants a CHANGELOG `patch` block, or already-scaffolded repos only get the change on a full re-run.
