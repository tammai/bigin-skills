...
- Guard test cases below are executable: `node tools/guard-tests/run.mjs` extracts every guard from `hook-guard.md` and runs both suites against the shipped templates (`regression.mjs` = single-agent behavior that must never change; `matrix.mjs` = the full legacy + scoped + team-gate matrix). Add a case there when you touch a guard; don't rely on the prose alone.
...
- `spec-gate-guard.mjs` is the other load-bearing gate, and it has two modes. Note `Owns:`/`Verified:` are read from the plan's **header region only** (before the first `##`), so a legacy spec body mentioning "Owns:" can't arm scoped mode. **Legacy** (no plan file declares `Owns:`) must stay byte-identical to its pre-agent-teams behavior — test: block a non-trivial edit with no/unapproved `PLAN.md`; allow once `Status: approved`; allow trivial paths; allow ≤20 lines. **Scoped** (some plan declares `Owns:`) — test: an approved owner allows; an unapproved owner blocks; an unowned path blocks *at any size* (the ≤20-line hatch must not apply); two plans at equal specificity block; a specific glob beats an `Owns: **` catch-all; `Owns:` with no globs blocks (fails closed); a path outside the repo blocks.
- `task-plan-gate.mjs` / `task-verify-gate.mjs` (agent teams) — **both must exit 0 when `.claude/task-plans/` is absent**, because the task tools fire in ordinary non-team sessions too; without it they'd break `TaskCreate` in every normal session. Test: no `task-plans` dir → no-op; missing `Plan:` ref blocks creation; `[coordination]` in the description always allowed; an unapproved or `Owns:`-less plan blocks creation; completion blocks without `Verified: PASS`; completion **allows** when the plan file is already gone (fails open — a task that can never complete deadlocks its dependents); unparseable stdin → no-op.
