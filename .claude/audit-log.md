...
Deferred (logged, not acted on):
- `[ -x tool/generate_api_client.sh ]` in the **flutter** CI templates (github + gitlab) skips the contract-diff gate when the script exists but has lost its executable bit — green forever behind an echo. Fixed in the two `tauri` templates this release; flutter's two are templated content that wants their own CHANGELOG `patch` block, so they were left deliberately rather than silently changed.
